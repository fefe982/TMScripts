...
(function () {
    ...
    $.fn.simTouchEvent = function (eveName, xoff, yoff) {
        ...
        customEvent.initMouseEvent(eveName, /*bubbles*/
            true, /*cancelable*/
            true, unsafeWindow /*view*/
        , /*detail*/
            1,
            x, y, x, y,
            false, false, false, false);
    ...
    var handler = {
        ...
        "12011562" : {
            ...
            get_actions : function () {
                return [
                    ...
                    [/*da2%2F*/
                        / useItemComplete /, 'aJ', '#bg > section > article > div > div > a'],
            ...
            handleERBBattle : function () {
                ...
                setInterval(function () {
                    ...
                    if (reload === false && recharge === true) { //getXPATH('//*[@id="do_battle_btn" and @style="display:none"]')) {
                        //debugger;
                        var ele,
                        ele_s; //*[@id="bp_recovery"]/div/div[2]
        ...
        "12008490" : {
            ...
            handleStrongBossTop : function () {
                ...
                var USERID = GM_getValue("__rg_USERID", "");
                GM_log(USERID);
                if (!succ) {
                    var ownerbox = $('#popup_content > div:nth-child(1) > div.section > div > div.no_flex > img');
                    var owner = false;
                    if (USERID != "" && ownerbox.attr('src') && ownerbox.attr('src').match(new RegExp(USERID))) {
                        owner = true;
                    }
                    var attacked = USERID != "" && $('#popup_content > div:nth-child(1) > div.section > ul > li > a[href$=' + USERID + ']').length > 0;
                    //debugger;
                    GM_log(url);
            ...
            get_actions : function () {
                return [
                    ...
                    [/mypage%2FIndex/, "func", function () {
                            ...
                            var succ = false;
                            //GM_log(txt[0].defaultValue);
                            var matchres = txt[0].defaultValue.match(/%3D(\d+)$/);
                            if (matchres) {
                                //GM_log(matchres[1]);
    ...
    var match_app_id = url.match(/http:\/\/sp\.pf\.mbga\.jp\/(\d+)/);
    if (match_app_id) {
        var action_handler = handler[match_app_id[1]];
        if (action_handler) {
